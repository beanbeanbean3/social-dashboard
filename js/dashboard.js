...
function getLast12Weeks() {
    ...
    const today = new Date();
        ...
        // Match PHP: "this sunday"
    const sunday = new Date(today);
    const day = sunday.getDay();

        // If today is Sunday, keep it.
        // Otherwise move to the coming Sunday.
    if (day !== 0) {
...
function aggregateModalData(weeksData) {
    ...
    weeksData.forEach(data => {
        ...
        for (const key in breakdown) {
            ...
            result.content_breakdown[key].views += Number(breakdown[key].views) || 0;
            result.views += Number(breakdown[key].views) || 0;
...
function createQuarterChart(platform, metric, labels, data) {
    ...
    const storageMap = {
        ...
        tiktok: ttQuarterCharts,
        youtube: ytQuarterCharts
    };
...
async function loadAllData() {
    ...
    try {
        ...
        loadPlatformData('linkedin'),
        loadPlatformData('tiktok'),
        loadPlatformData('youtube'),
        fetchYearlyData('facebook'),
        fetchYearlyData('instagram'),
        fetchYearlyData('twitter'),
        fetchYearlyData('linkedin'),
        fetchYearlyData('tiktok'),
        fetchYearlyData('youtube')
    ]);
    ...
    document.getElementById('tw-reach').innerHTML =
    formatValue(twData?.reach || 0);

    document.getElementById('tw-engagements').innerHTML =
    formatValue(twData?.engagements || 0);

    document.getElementById('tw-followers').innerHTML =
    formatValue(
        twData?.new_followers ?? twData?.followers ?? 0,
        { isFollower: true }
        );

    document.getElementById('tw-visits').innerHTML =
    formatValue(twData?.visits || 0);

    document.getElementById('tw-year-reach').innerHTML =
    formatValue(twYear?.reach || 0);

    document.getElementById('tw-year-engagements').innerHTML =
    formatValue(twYear?.engagements || 0);

    document.getElementById('tw-year-followers').innerHTML =
    formatValue(
        twYear?.new_followers ?? twYear?.followers ?? 0,
        { isFollower: true }
        );

    document.getElementById('tw-year-visits').innerHTML =
    formatValue(twYear?.visits || 0);
    ...
    document.getElementById('li-reach').innerHTML =
    formatValue(liData?.reach || 0);

    document.getElementById('li-engagements').innerHTML =
    formatValue(liData?.engagements || 0);

    document.getElementById('li-followers').innerHTML =
    formatValue(
        liData?.new_followers ?? liData?.followers ?? 0,
        { isFollower: true }
        );

    document.getElementById('li-visits').innerHTML =
    formatValue(liData?.visits || 0);

    document.getElementById('li-year-reach').innerHTML =
    formatValue(liYear?.reach || 0);

    document.getElementById('li-year-engagements').innerHTML =
    formatValue(liYear?.engagements || 0);

    document.getElementById('li-year-followers').innerHTML =
    formatValue(
        liYear?.new_followers ?? liYear?.followers ?? 0,
        { isFollower: true }
        );

    document.getElementById('li-year-visits').innerHTML =
    formatValue(liYear?.visits || 0);
    ...
    document.getElementById('tt-reach').innerHTML =
    formatValue(ttData?.reach || 0);

    document.getElementById('tt-engagements').innerHTML =
    formatValue(ttData?.engagements || 0);

    document.getElementById('tt-followers').innerHTML =
    formatValue(
        ttData?.new_followers ?? ttData?.followers ?? 0,
        { isFollower: true }
        );

    document.getElementById('tt-visits').innerHTML =
    formatValue(ttData?.visits || 0);

    document.getElementById('tt-year-reach').innerHTML =
    formatValue(ttYear?.reach || 0);

    document.getElementById('tt-year-engagements').innerHTML =
    formatValue(ttYear?.engagements || 0);

    document.getElementById('tt-year-followers').innerHTML =
    formatValue(
        ttYear?.new_followers ?? ttYear?.followers ?? 0,
        { isFollower: true }
        );

    document.getElementById('tt-year-visits').innerHTML =
    formatValue(ttYear?.visits || 0);
    ...
    document.getElementById('yt-reach').innerHTML =
    formatValue(ytData?.reach || 0);

    document.getElementById('yt-engagements').innerHTML =
    formatValue(ytData?.engagements || 0);

    document.getElementById('yt-followers').innerHTML =
    formatValue(
        ytData?.new_followers ?? ytData?.followers ?? 0,
        { isFollower: true }
        );

    document.getElementById('yt-visits').innerHTML =
    formatValue(ytData?.visits || 0);

    document.getElementById('yt-year-reach').innerHTML =
    formatValue(ytYear?.reach || 0);

    document.getElementById('yt-year-engagements').innerHTML =
    formatValue(ytYear?.engagements || 0);

    document.getElementById('yt-year-followers').innerHTML =
    formatValue(
        ytYear?.new_followers ?? ytYear?.followers ?? 0,
        { isFollower: true }
        );

    document.getElementById('yt-year-visits').innerHTML =
    formatValue(ytYear?.visits || 0);
    ...
    await Promise.all([
        ...
        loadQuarterlyCharts('tiktok'),
        loadQuarterlyCharts('youtube')
    ]);
...
async function openInsightsModal(platform) {
        ...
        const titles = {
            facebook: "Facebook Content Overview",
            instagram: "Instagram Content Overview",
            twitter: "Twitter Content Overview",
            linkedin: "LinkedIn Content Overview",
            tiktok: "TikTok Content Overview",
            youtube: "YouTube Content Overview"
        };

        title.innerText = titles[platform] || "Content Overview";
    ...
    function loadModalChart(platform, type="all") {
        ...
        modalDataCache.raw.forEach((weekData, index) => {
            ...
            if(type === "all"){
                ...
                for(const key in breakdown){
                    views += Number(breakdown[key].views) || 0;
                }
            } else {
                const breakdown = weekData.content_breakdown || {};
        ...
        for(const key in breakdown){
            ...
            if(type !== "all"){
                ...
                if(platform === "youtube"){
                    ...
                    if(type === "videos" && key !== "Videos") continue;
    ...
    function renderTopContent(posts, type="all"){
        ...
        posts.forEach(post=>{
           ...
           if(type !== "all"){
            ...
            if(getActivePlatform() === "youtube"){

                if(type === "videos" && postType !== "videos") return;

            }else{

                if(type === "posts" && !["posts","carousel"].includes(postType)) return;
                if(type === "reels" && !["videos","reels"].includes(postType)) return;
